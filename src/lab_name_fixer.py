...
class LabNameFixer:
    ...
    FIELD_PATTERN = re.compile(r"实验[（(]训[）)]室名称\s*[：:]")
    
    def __init__(self):
        pass
    
    def extract_lab_name(self, file_path: str) -> str:
        """
        提取文档中的实验室名称
        ...
        try:
            ...
            return self.extract_lab_name_from_doc(doc)
        except Exception:
            return ""
    
    def extract_lab_name_from_doc(self, doc) -> str:
        """
        从已打开的 Document 对象中提取实验室名称
        ...
        for paragraph in self._iter_all_paragraphs(doc):
            lab_name = self._extract_from_paragraph(paragraph)
            if lab_name:
                return lab_name
    ...
    def batch_fix_lab_name(self, folder_path: str, new_lab_name: str) -> Tuple[List[str], List[str]]:
        """
        批量修正实验室名称
        
        Args:
            folder_path: 文件夹路径
            new_lab_name: 新的实验室名称（已去除首尾空格）
        ...
        for file_path in docx_files:
            try:
                success = self._fix_single_file(file_path, new_lab_name)
                if success:
    ...
    def _fix_single_file(self, file_path: str, new_lab_name: str) -> bool:
        """
        修正单个文件的实验室名称
        
        Args:
            file_path: 文件路径
            new_lab_name: 新的实验室名称
        ...
        for paragraph in self._iter_all_paragraphs(doc):
            if self._contains_field(paragraph.text):
                self._replace_text_after_label(paragraph, new_lab_name)
                modified = True
    ...
    def _replace_text_after_label(self, paragraph, new_lab_name: str):
        """
        替换字段名后面的内容，保留原格式
        
        策略（简化版，避免run拆分导致的位置计算错误）：
        1. 找到字段名
        2. 清空所有run
        3. 在第一个run写入"字段名+新名称"（保留第一个run的格式）
        ...
        new_text = full_text[:m.end()] + new_lab_name + rest[cut:]
